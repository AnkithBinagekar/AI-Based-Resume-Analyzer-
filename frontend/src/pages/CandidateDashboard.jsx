...
// Paste this right above function CandidateDashboard() { ... }
...
function CandidateDashboard() {
  ...
  useEffect(() => { fetchJobs(); }, []);
  useEffect(() => {
    if (location.state?.selectedJobId && dbJobs.length > 0) { setSelectedSavedJobId(location.state.selectedJobId); setJdMode('saved'); }
  ...
  const fetchJobs = async () => {
    try {
      ...
      if (response.data.data.length > 0 && !selectedSavedJobId && !location.state?.selectedJobId) setSelectedSavedJobId(response.data.data[0].id);
  ...
  const handleAnalyze = async (e) => {
    ...
    try {
      ...
      if (uploadMode === 'single') setSingleResults(response.data.data); else setBulkResults(response.data.data);
    } catch (err) {
  ...
  const generateXAIReasons = (results) => {
    ...
    if (lexical > (semantic + 0.3)) reasons.push({ type: 'danger', text: "Lexical Anomaly: Exact keyword matches are unusually high compared to semantic meaning. (Potential Keyword Stuffing)." });
    else if (lexical < 0.2 && semantic > 0.5) reasons.push({ type: 'positive', text: "Excellent vocabulary variance. Candidate explains concepts well without copying the JD wording." });
                ...
                <div className="p-8">
                  {resultTab === 'overview' && (
                    <div ref={reportRef} id="candidate-report-content" className="space-y-10 bg-transparent p-2 print-color-adjust-exact">
                      <div className="hidden print:block mb-8 border-b-2 border-[#374151] pb-6">
                        <div className="flex justify-between items-end">
                          <div>
                            <h1 className="text-2xl font-black text-[#F7F9FC] tracking-tight">AI Candidate Intelligence Report</h1>
                            <p className="text-sm font-bold text-[#94A3B8] mt-1">Generated for Job #{singleResults.job_id || 'Custom'}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-xs font-bold text-[#94A3B8] uppercase tracking-widest">Generated By</p>
                            <p className="text-sm font-bold text-[#F7F9FC]">IntelligenceATS</p>
                          </div>
                        </div>
                      </div>
                      ...
                      {singleResults?.smart_alerts && singleResults.smart_alerts.length > 0 && (
                        <div className="space-y-4 animate-in fade-in duration-500">
                          {singleResults.smart_alerts.map((alert, index) => (
                            <div key={index} className={`p-5 rounded-2xl border backdrop-blur-md flex items-start gap-4 shadow-sm ${alert.type === 'warning' ? 'bg-[#F59E0B]/20 border-[#F59E0B]/30 text-[#F59E0B]' : 'bg-[#E85D75]/20 border-[#E85D75]/30 text-[#E85D75]'}`}>
                              <div className="mt-0.5 text-2xl drop-shadow-md">{alert.type === 'warning' ? '⚠️' : '🚨'}</div>
                              <div>
                                <h4 className="font-bold text-sm tracking-wide uppercase drop-shadow-sm">{alert.title}</h4>
                                <p className="text-sm font-medium mt-1 opacity-90 leading-relaxed">{alert.message}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                        {[
                          ...
                          <div key={idx} className={`p-5 rounded-2xl border text-center transition-transform hover:scale-105 shadow-sm bg-[#1A1F2E]/50 backdrop-blur-md border-white/5`}>
                            <h4 className={`text-[10px] font-black uppercase mb-2 tracking-widest`} style={{color: item.color}}>{item.label}</h4>
                            <p className={`font-black drop-shadow-sm ${item.truncate ? 'truncate text-sm text-[#F7F9FC]' : 'text-2xl text-[#F7F9FC]'}`} title={item.value}>{item.value}</p>
                          </div>
                        ))}
                      </div>

                     <div className="flex flex-col items-center p-8 bg-[#1A1F2E]/40 backdrop-blur-md rounded-3xl border border-white/5 shadow-inner">
  <h3 className="text-xs font-black text-[#94A3B8] uppercase tracking-widest mb-8">Explainable AI Radar</h3>
  
  {/* Changed rounded-full to rounded-3xl and added w-full max-w-lg */}
  <div className="bg-[#242B3D]/80 backdrop-blur-xl p-6 md:p-8 w-full max-w-lg rounded-3xl border border-white/10 shadow-lg">
     <XAIDial featureBreakdown={singleResults.feature_breakdown} />
                  ...
                  {resultTab === 'logic' && (
                    <div className="bg-[#1A1F2E]/50 backdrop-blur-md p-8 rounded-3xl border border-white/5 shadow-inner space-y-8">
                      <div>
                      ...
                      <div className="bg-[#242B3D]/60 backdrop-blur-xl p-8 rounded-2xl border border-white/5 shadow-lg">
                         <h4 className="text-xs font-black uppercase text-[#94A3B8] tracking-widest mb-6 flex items-center gap-2"><span>👁️‍🗨️</span> Live AI Reasoning</h4>
                         <ul className="space-y-4">
                           {generateXAIReasons(singleResults).map((reason, idx) => (
                             <li key={idx} className="flex items-start gap-4 text-sm font-medium p-5 rounded-xl bg-[#1A1F2E]/80 backdrop-blur-md border border-white/5 shadow-inner">
                               ...
                               {reason.type === 'danger' && <span className="text-[#E85D75] text-xl mt-0.5 animate-pulse drop-shadow-[0_0_8px_rgba(232,93,117,0.9)]">🚨</span>}
                           ...
                           {generateXAIReasons(singleResults).length === 0 && <li className="text-sm text-[#94A3B8] italic p-4 bg-[#1A1F2E]/80 rounded-xl border border-white/5">Average candidate profile. No anomalies.</li>}
                         </ul>
                      </div>
                    </div>
                  )}
