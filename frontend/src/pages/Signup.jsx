...
function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    } catch (err) { setError(err.response?.data?.detail || "Failed to create account."); } finally { setLoading(false); }
  };
  ...
  const handleDemoLogin = async () => {
    ...
    } catch (err) { setError("Demo login failed."); setLoading(false); }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#1A1F2E] relative overflow-hidden p-6 font-sans">
      ...
      <Link to="/" className="absolute top-6 left-6 md:top-10 md:left-10 flex items-center gap-2 text-[#F7F9FC] hover:text-[#2F6FED] transition-all z-50 group">
        <span className="text-2xl text-[#2F6FED] drop-shadow-[0_0_15px_rgba(47,111,237,0.6)] group-hover:scale-110 transition-transform">⚡</span>
      ...
      <div className="w-full max-w-md bg-[#242B3D]/40 backdrop-blur-2xl p-10 rounded-3xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)] relative z-10">
        <div className="text-center mb-10">
          <div className="text-4xl text-[#2F6FED] mb-4 drop-shadow-[0_0_20px_rgba(47,111,237,0.5)]">⚡</div>
          <h2 className="text-2xl font-black text-[#F7F9FC]">Create Workspace</h2>
          <p className="text-[#94A3B8] text-sm font-medium mt-2">Get started with IntelligenceATS</p>
        </div>
        ...
        <button onClick={handleDemoLogin} className="mt-6 w-full bg-[#1A1F2E]/50 backdrop-blur-md hover:bg-[#242B3D]/80 text-[#F7F9FC] border border-white/5 font-bold py-3.5 rounded-xl transition-all active:scale-[0.98] text-sm shadow-sm flex items-center justify-center gap-2">
        ...
        <p className="text-center mt-8 text-sm font-medium text-[#94A3B8]">
          Already have an account? <Link to="/login" className="text-[#2F6FED] hover:text-[#2F6FED]/80 font-bold drop-shadow-sm">Sign in</Link>
        </p>
      </div>
    </div>
  );
